...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagsteer1)
    {
        info->event[info->num] = EVENT_steer1;
        info->num++;
        dataPoints->valuesteer1 = gizX2Y(steer1_RATIO,  steer1_ADDITION, issuedData->attrVals.valuesteer1); 
    }
        
    if(0x01 == issuedData->attrFlags.flagsteer2)
    {
        info->event[info->num] = EVENT_steer2;
        info->num++;
        dataPoints->valuesteer2 = gizX2Y(steer2_RATIO,  steer2_ADDITION, issuedData->attrVals.valuesteer2); 
    }
        
    if(0x01 == issuedData->attrFlags.flagsteer3)
    {
        info->event[info->num] = EVENT_steer3;
        info->num++;
        dataPoints->valuesteer3 = gizX2Y(steer3_RATIO,  steer3_ADDITION, issuedData->attrVals.valuesteer3); 
    }
        
    if(0x01 == issuedData->attrFlags.flagsteer4)
    {
        info->event[info->num] = EVENT_steer4;
        info->num++;
        dataPoints->valuesteer4 = gizX2Y(steer4_RATIO,  steer4_ADDITION, issuedData->attrVals.valuesteer4); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuesteer1 != cur->valuesteer1)
    {
        GIZWITS_LOG("valuesteer1 Changed\n");
        ret = 1;
    }
    if(last->valuesteer2 != cur->valuesteer2)
    {
        GIZWITS_LOG("valuesteer2 Changed\n");
        ret = 1;
    }
    if(last->valuesteer3 != cur->valuesteer3)
    {
        GIZWITS_LOG("valuesteer3 Changed\n");
        ret = 1;
    }
    if(last->valuesteer4 != cur->valuesteer4)
    {
        GIZWITS_LOG("valuesteer4 Changed\n");
        ret = 1;
    }

    if(last->valuestate1 != cur->valuestate1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuestate1 Changed\n");
            ret = 1;
        }
    }
    if(last->valuestate2 != cur->valuestate2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuestate2 Changed\n");
            ret = 1;
        }
    }
    if(last->valuestate3 != cur->valuestate3)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuestate3 Changed\n");
            ret = 1;
        }
    }
    if(last->valuestate4 != cur->valuestate4)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuestate4 Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuesteer1 = gizY2X(steer1_RATIO,  steer1_ADDITION, dataPoints->valuesteer1); 
    devStatusPtr->valuesteer2 = gizY2X(steer2_RATIO,  steer2_ADDITION, dataPoints->valuesteer2); 
    devStatusPtr->valuesteer3 = gizY2X(steer3_RATIO,  steer3_ADDITION, dataPoints->valuesteer3); 
    devStatusPtr->valuesteer4 = gizY2X(steer4_RATIO,  steer4_ADDITION, dataPoints->valuesteer4); 
    devStatusPtr->valuestate1 = gizY2X(state1_RATIO,  state1_ADDITION, dataPoints->valuestate1); 
    devStatusPtr->valuestate2 = gizY2X(state2_RATIO,  state2_ADDITION, dataPoints->valuestate2); 
    devStatusPtr->valuestate3 = gizY2X(state3_RATIO,  state3_ADDITION, dataPoints->valuestate3); 
    devStatusPtr->valuestate4 = gizY2X(state4_RATIO,  state4_ADDITION, dataPoints->valuestate4); 
